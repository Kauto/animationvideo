...
export default class ImageFrame implements IAnimation {
  ...
  constructor(
    frameNumber: OrFunction<number | number[]>,
    framesToRight: OrFunction<boolean>,
    durationBetweenFrames: OrFunction<number>,
  ) {
    ...
    this._frameNumber = Array.isArray(frameNumberCalc)
      ? frameNumberCalc
      : [frameNumberCalc];
  ...
  run(sprite: ISprite & {p: {frameX?: number, frameY?: number, frameWidth?: number, frameHeight?: number}}, time: number) {
    let currentFrame = 0;
    if (time >= this._duration) {
      currentFrame = this._frameNumber[this._frameNumber.length - 1];
    } else {
      currentFrame =
        this._frameNumber[Math.floor(time / this._durationBetweenFrames)];
    }
