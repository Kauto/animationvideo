...
export default class If implements IAnimation {
  ...
  constructor(
    ifCallback: OrFunction<boolean>,
    Aniobject: IAnimation | TAnimationFunction,
    AniobjectElse: IAnimation | TAnimationFunction,
  ) {
  ...
  play(label = "", timelapsed = 0) {
    return (
      (this._Aniobject as IAnimation).play?.(label, timelapsed) ||
      (this._AniobjectElse as IAnimation).play?.(label, timelapsed)
    );
  }
