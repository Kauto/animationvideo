...
> {
  ...
  constructor(
    ...
    this._drawFrame = Array.from({ length: this._canvasCount }, (_v) => 0);
  ...
  _mainLoop(timestamp: number) {
    ...
    if (this._scene) {
      ...
      if (!this._reduceFramerate || this._isOddFrame) {
        const now = this._scene.currentTime();
      ...
      if (
        ...
        if (this._autoSize.currentWaitedTime < this._autoSize.waitTime) {
          ...
          const targetTime =
            this._autoSize.offsetTimeTarget! * (this._reduceFramerate ? 2 : 1);
