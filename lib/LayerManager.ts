...
class LayerManager {
  ...
  addLayers(
    numberOfLayer: number = 1,
    canvasIds: undefined | number | number[] = undefined,
  ) {
    const newLayers = Array.from(
      { length: numberOfLayer },
      (_v) => new Layer(canvasIds),
    );
  ...
  addLayersForIds(
    ...
    const result = Array.from(
      { length: numberOfLayer },
      (_v, k) => k + this._layers.length,
    );
    this._layers = this._layers.concat(
      Array.from({ length: numberOfLayer }, (_v) => new Layer(canvasIds)),
    );
    return result;
  }
