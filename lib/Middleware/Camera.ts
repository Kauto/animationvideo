...
export default class Camera implements ConfigurationObject {
  ...
  viewport(_: ParameterListWithoutTime, matrix: Transform) {
    return matrix
  ...
  clampView(
    params: ParameterListWithoutTime & { clampLimits?: RectPosition },
    cam: CameraPosition,
  ) {
  ...
  zoomTo(
    params: ParameterListWithoutTime & RectPosition & { cam?: CameraPosition },
  ) {
    ...
    const ret = {
      ...
      zoom:
        (cam || this.cam).zoom *
        Math.max(Math.min(zoomX, zoomY), Number.MIN_VALUE),
    };
    if (cam) {
      cam.x = ret.x;
      cam.y = ret.y;
