...
export default class CameraControl implements ConfigurationObject {
  ...
  mouseMove(props: ParameterListPositionEvent) {
    ...
    if ((e as TouchEvent).touches?.length >= 2) {
      ...
      const viewMatrix = scene.camera.viewportByCam(props, this.toCam).invert();
  ...
  mouseWheel(props: ParameterListPositionEvent) {
    const {
      event: e,
      position: [mx, my],
      scene,
    ...
    const wheelData =
      // @ts-expect-error wheelDelta is old ff-api
      (e as WheelEvent).wheelDelta || (e as WheelEvent).deltaY * -1;
    if (wheelData / 120 > 0) {
      this.zoomIn();
  ...
  hasCamChanged() {
    ...
    return (
      Math.abs(this.toCam.x - this._scene!.camera.cam.x) >=
        Number.EPSILON * t ||
      Math.abs(this.toCam.y - this._scene!.camera.cam.y) >=
        Number.EPSILON * t ||
  ...
  zoomTo(params: ParameterListWithoutTime & RectPosition) {
    params.scene.camera.zoomTo(Object.assign(params, { cam: this.toCam }));
  }
}
