import Scene, {
  ConfigurationObject,
  type EventsReturn,
  type ParameterListInitDestroy,
  type ParameterListWithoutTime,
} from "../Scene";
...
export default class Events implements ConfigurationObject {
  ...
  _pushEvent(
    command: "mouseDown" | "mouseUp" | "mouseOut" | "mouseMove" | "mouseWheel",
  ...
  events({ scene }: ParameterListWithoutTime) {
    return [
      scene.has("mouseDown") && [
        ["touchstart", "mousedown"],
        (event: TouchEvent | MouseEvent) =>
          this._pushEvent("mouseDown", event, scene),
      ],
      scene.has("mouseUp") && [
        ["touchend", "mouseup"],
        (event: TouchEvent | MouseEvent) =>
          this._pushEvent("mouseUp", event, scene),
      ],
      scene.has("mouseOut") && [
        ["touchendoutside", "mouseout"],
        (event: TouchEvent | MouseEvent) =>
          this._pushEvent("mouseOut", event, scene),
      ],
      scene.has("mouseMove") && [
        ["touchmove", "mousemove"],
        (event: TouchEvent | MouseEvent) =>
          this._pushEvent("mouseMove", event, scene),
      ],
  ...
  init({ output, scene }: ParameterListInitDestroy) {
    ...
    )[][] = scene.map<Events, "events">("events", {});
    ...
    this._events = events
      ...
      .map((cur) =>
        Array.isArray(cur)
          ...
          : [
              [cur],
              (event: ValueOf<HTMLElementEventMap>) => {
  ...
  reset(
    _params: ParameterListWithoutTime,
