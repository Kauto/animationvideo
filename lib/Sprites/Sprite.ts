...
export type ISpriteWithPosition = ISprite & { p: { x: number; y: number } };
...
export class SpriteBase<
  O extends SpriteBaseOptions = SpriteBaseOptions,
  P extends SpriteBaseOptionsInternal = SpriteBaseOptionsInternal,
> implements ISprite
{
  _needInit = true;
  p: P;
  ...
  _parseParameterList(
    parameterList: TParameterList<O, P>,
    givenParameter: O,
  ): P {
    const parameterEntries = Object.entries(
      parameterList as ReturnType<this["_getParameterList"]>,
    );
    const valueEntries = parameterEntries.map(([name, d]) => {
  ...
  _getBaseParameterList() {
    return {
      // animation
      animation: (
        value: SpriteBaseOptions["animation"],
        _givenParameter: O,
      ) => {
      ...
      tag: (value: SpriteBaseOptions["tag"], _givenParameter: O) => {
  ...
  _detectHelper(
    ...
    callback?: (hw: number, hh: number) => boolean,
  ) {
    let a = false;
  ...
  detectDraw(_context: CanvasRenderingContext2D, _color: string) {}
