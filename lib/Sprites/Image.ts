...
export interface SpriteImageOptionsInternal
  extends SpriteCircleOptionsInternal {
...
class Image extends SpriteBase<SpriteImageOptions, SpriteImageOptionsInternal> {
  ...
  _getParameterList() {
    return Object.assign({}, super._getParameterList(), CircleParameterList, {
      // set image
      image: (v: OrFunction<HTMLImageElement | string>) =>
        ImageManager.getImage(calc(v)),
      // relative position
  ...
  detectDraw(context: CanvasRenderingContext2D, color: string) {
    ...
    if (p.enabled && p.isClickable && p.clickExact) {
      ...
      const sY =
        (p.height ? p.height : frameHeight) * this._normScale! * p.scaleY;
