...
export interface SpriteStarFieldOptionsInternal
...
// Sprite
// Draw a Circle
export default class StarField extends SpriteBase<
  SpriteStarFieldOptions,
  SpriteStarFieldOptionsInternal
> {
  _starsX: number[] = [];
  _starsY: number[] = [];
  _starsZ: number[] = [];
  ...
  animate(timepassed: number) {
    const ret = super.animate(timepassed);
      ...
      if (p.moveY == 0 && p.moveZ == 0 && p.moveX < 0) {
        ...
        while (i--) {
          if (this._starsEnabled[i]) {
            context.fillRect(
              this._starsNewX[i],
              this._starsNewY[i] - (this._starsLineWidth[i] * p.lineWidth) / 2,
