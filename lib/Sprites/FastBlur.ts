...
export interface SpriteFastBlurOptionsInternal
...
export default class FastBlur<
  O extends SpriteFastBlurOptions = SpriteFastBlurOptions,
  P extends SpriteFastBlurOptionsInternal = SpriteFastBlurOptionsInternal,
> extends SpriteBase<O, P> {
  ...
  _getParameterList() {
    return Object.assign({}, super._getParameterList(), {
      ...
      clear: false,
      norm: (
        value: SpriteFastBlurOptions["norm"],
        givenParameter: SpriteFastBlurOptions,
      ) =>
        ifNull(
          calc(value),
      ...
      // scalling
      scaleX: (
        value: SpriteFastBlurOptions["scaleX"],
        givenParameter: SpriteFastBlurOptions,
      ) => {
        return ifNull(calc(value), ifNull(calc(givenParameter.scale), 1));
      },
      scaleY: (
        value: SpriteFastBlurOptions["scaleY"],
        givenParameter: SpriteFastBlurOptions,
      ) => {
        return ifNull(calc(value), ifNull(calc(givenParameter.scale), 1));
      },
  ...
  _generateTempCanvas(additionalModifier: AdditionalModifier) {
    ...
    // @ts-expect-error willReadFrequently is a new property and not known by typescript for now
    this._tctx.willReadFrequently = true;
  ...
  draw(
    ...
    if (p.enabled && p.alpha > 0) {
      ...
      if (a > 0 && targetW && targetH) {
        ...
        if (
          "additionalBlur" in this &&
          typeof this.additionalBlur === "function"
        ) {
