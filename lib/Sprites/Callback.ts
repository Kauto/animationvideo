...
export type SpriteCallback = (
  context: CanvasRenderingContext2D,
  timePassed: number,
  additionalParameter: AdditionalModifier,
  sprite: ISprite,
) => void;
...
export interface SpriteCallbackOptionsInternal
  extends SpriteBaseOptionsInternal {
  callback: SpriteCallback;
}

export default class Callback extends SpriteBase<
  SpriteCallbackOptions,
  SpriteCallbackOptionsInternal
> {
  _timePassed = 0;

  constructor(
    givenParameter: SpriteCallbackOptions | SpriteCallbackOptions["callback"],
  ) {
  ...
  _getParameterList() {
    return Object.assign({}, this._getBaseParameterList(), {
      callback: (v: SpriteCallbackOptions["callback"]) =>
        (typeof v === "undefined" ? () => {} : v) as SpriteCallback,
    });
  }
